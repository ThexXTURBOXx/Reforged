	...
	protected boolean onEntityHit(Entity entity) {
		...
		if (!entity.isDead) {
			...
			if (entity instanceof EntityLivingBase) {
				...
				case "hunger":
					p.addPotionEffect(new PotionEffect(Potion.hunger.getId(), 300, 1));
					break;

				case "poison":
					p.addPotionEffect(new PotionEffect(Potion.poison.getId(), 200, 1));
					break;

				case "poison_strong":
					p.addPotionEffect(new PotionEffect(Potion.poison.getId(), 300, 2));
					break;

				case "slowness":
					p.addPotionEffect(new PotionEffect(Potion.moveSlowdown.getId(), 300, 1));
					p.addPotionEffect(new PotionEffect(Potion.digSlowdown.getId(), 300, 1));
					break;

				case "wither":
					p.addPotionEffect(new PotionEffect(Potion.wither.getId(), 300, 1));
					break;
