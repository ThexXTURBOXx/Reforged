...
public class EntityBoomerang extends AReforgedThrowable {
	...
	protected boolean onBlockHit(BlockPos blockPos) {
		...
		if (!worldObj.isRemote) {
			if (getItemStack().getMaxDamage() - getItemStack().getItemDamage() > 0 && !creativeUse()) {
				...
				worldObj.playSoundAtEntity(this, "reforged:boomerang_hit", 2.0F, 1.0F);
			} else if (getItemStack().getMaxDamage() - getItemStack().getItemDamage() <= 0) {
				worldObj.playSoundAtEntity(this, "reforged:boomerang_break", 2.0F, 1.0F);
			} else if (creativeUse()) {
				worldObj.playSoundAtEntity(this, "reforged:boomerang_hit", 2.0F, 1.0F);
			}
		}
	...
	public void readEntityFromNBT(NBTTagCompound tagCompund) {
		...
		dataWatcher.updateObject(9, tagCompund.getFloat("yawreforged"));
	}
	...
	public void writeEntityToNBT(NBTTagCompound tagCompound) {
		...
		tagCompound.setFloat("yawreforged", dataWatcher.getWatchableObjectFloat(9));
